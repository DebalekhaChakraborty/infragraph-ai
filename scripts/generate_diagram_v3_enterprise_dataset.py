#!/usr/bin/env python3
"""
generate_diagram_v3_enterprise_dataset.py
...
Usage:
    python scripts/generate_diagram_v3_enterprise_dataset.py \
        --num-scenarios 100 --out ./datasets/diagram_v3_enterprise \
        --seed 2026 --clean
"""
...
def _branch_template(rng, scenario_idx, diagram_types):
    ...
    primary = [
        {"id": "BR-RTR-01", "type": "router",   "zone": "Branch Edge", "pos": (0.20, 0.48),
         "ip_zone": "branch", "is_shared": True,  "canonical_id": "BR-RTR-01"},
        {"id": "BR-FW-01",  "type": "firewall", "zone": "Branch Edge", "pos": (0.35, 0.48),
         "ip_zone": "branch", "is_shared": False, "canonical_id": "BR-FW-01"},
        {"id": "BR-SW-01",  "type": "switch",   "zone": "Branch LAN",  "pos": (0.52, 0.48),
         "ip_zone": "branch", "is_shared": False, "canonical_id": "BR-SW-01"},
        {"id": "BR-APP-01", "type": "server",   "zone": "Branch Apps", "pos": (0.72, 0.34),
         "ip_zone": "branch", "is_shared": False, "canonical_id": "BR-APP-01"},
    ]
    ys = [0.64, 0.78, 0.50, 0.28][:n_wrk]
    for i, y in enumerate(ys, 1):
        primary.append({"id": f"BR-WRK-{i:02d}", "type": "server", "zone": "Branch LAN",
                         "pos": (0.72 + (i % 2) * 0.14, y), "ip_zone": "branch", "is_shared": False,
                         "canonical_id": f"BR-WRK-{i:02d}"})
    ghost = []
    if "wan_topology" in diagram_types:
        ghost.append({"id": "WAN-MPLS-CORE", "type": "cloud_or_wan", "zone": "WAN",
                      "pos": (0.08, 0.48), "is_shared": True, "canonical_id": "WAN-MPLS-CORE"})
    edges = [
        {"src": "WAN-MPLS-CORE", "dst": "BR-RTR-01", "rel": "wan_dependency", "label": "MPLS", "is_cross": True},
    ...
    for i in range(1, n_wrk + 1):
        edges.append({"src": "BR-SW-01", "dst": f"BR-WRK-{i:02d}", "rel": "connected_to",
                       "label": "Access", "is_cross": False})
    zones = [
        {"name": "WAN Handoff", "color": "#EBF5FB", "bounds": (0.03, 0.23, 0.15, 0.72)},
        {"name": "Branch Edge", "color": "#FEF9E7", "bounds": (0.16, 0.22, 0.45, 0.72)},
        {"name": "Branch LAN",  "color": "#EAFAF1", "bounds": (0.46, 0.22, 0.93, 0.86)},
        {"name": "Branch Apps", "color": "#EBF5FB", "bounds": (0.64, 0.16, 0.93, 0.44)},
    ]
...
def _wan_template(rng, scenario_idx, diagram_types):
    primary = [
        {"id": "WAN-ISP-01",    "type": "cloud_or_wan", "zone": "Internet/ISP", "pos": (0.08, 0.30),
         "ip_zone": "wan", "is_shared": False, "canonical_id": "WAN-ISP-01"},
        {"id": "WAN-ISP-02",    "type": "cloud_or_wan", "zone": "Internet/ISP", "pos": (0.08, 0.67),
         "ip_zone": "wan", "is_shared": False, "canonical_id": "WAN-ISP-02"},
        {"id": "WAN-MPLS-CORE", "type": "cloud_or_wan", "zone": "WAN Core",     "pos": (0.43, 0.48),
         "ip_zone": "wan", "is_shared": False, "canonical_id": "WAN-MPLS-CORE"},
        {"id": "WAN-PE-01",     "type": "router",       "zone": "WAN PE",       "pos": (0.26, 0.34),
         "ip_zone": "wan", "is_shared": False, "canonical_id": "WAN-PE-01"},
        {"id": "WAN-PE-02",     "type": "router",       "zone": "WAN PE",       "pos": (0.62, 0.62),
         "ip_zone": "wan", "is_shared": False, "canonical_id": "WAN-PE-02"},
    ]
    ghost = []
    edges = [
        {"src": "WAN-ISP-01",    "dst": "WAN-MPLS-CORE", "rel": "routes_to", "label": "BGP",  "is_cross": False},
        {"src": "WAN-ISP-02",    "dst": "WAN-MPLS-CORE", "rel": "routes_to", "label": "BGP",  "is_cross": False},
        {"src": "WAN-MPLS-CORE", "dst": "WAN-PE-01",     "rel": "routes_to", "label": "MPLS", "is_cross": False},
        {"src": "WAN-MPLS-CORE", "dst": "WAN-PE-02",     "rel": "routes_to", "label": "MPLS", "is_cross": False},
    ]
    if "branch_topology" in diagram_types:
        ghost.append({"id": "BR-RTR-01", "type": "router",   "zone": "Branch",
                       "pos": (0.26, 0.72), "is_shared": True, "canonical_id": "BR-RTR-01"})
        edges.append({"src": "WAN-PE-01", "dst": "BR-RTR-01", "rel": "connected_to",
                       "label": "L3VPN", "is_cross": True})
    if "datacenter_topology" in diagram_types:
        ghost.append({"id": "DC-RTR-01", "type": "router", "zone": "DC Edge",
                       "pos": (0.78, 0.40), "is_shared": True, "canonical_id": "DC-RTR-01"})
        ghost.append({"id": "DC-FW-01", "type": "firewall", "zone": "DC Edge",
                       "pos": (0.86, 0.58), "is_shared": True, "canonical_id": "DC-FW-01"})
        edges.append({"src": "WAN-PE-02", "dst": "DC-RTR-01",  "rel": "connected_to",
                       "label": "L3VPN", "is_cross": True})
        edges.append({"src": "DC-RTR-01", "dst": "DC-FW-01",  "rel": "routes_to",
                       "label": "DC Edge", "is_cross": True})
    zones = [
        {"name": "Internet/ISP", "color": "#EBF5FB", "bounds": (0.03, 0.16, 0.17, 0.78)},
        {"name": "Provider Edge", "color": "#FEF9E7", "bounds": (0.18, 0.18, 0.34, 0.82)},
        {"name": "WAN Core",     "color": "#EAF2F8", "bounds": (0.36, 0.20, 0.68, 0.74)},
        {"name": "Enterprise Edge", "color": "#EAFAF1", "bounds": (0.70, 0.24, 0.94, 0.70)},
    ]
    return primary, ghost, edges, zones
...
def _datacenter_template(rng, scenario_idx, diagram_types):
    ...
    primary = [
        {"id": "DC-FW-01",      "type": "firewall", "zone": "DC Edge",        "pos": (0.20, 0.38),
         "ip_zone": "datacenter", "is_shared": True,  "canonical_id": "DC-FW-01"},
        {"id": "DC-FW-02",      "type": "firewall", "zone": "DC Edge",        "pos": (0.20, 0.64),
        ...
        {"id": "DC-CORE-SW-01", "type": "switch",   "zone": "DC Core",        "pos": (0.40, 0.38),
    ...
    srv_ys = [0.22, 0.38, 0.54, 0.70, 0.82][:n_srv]
    for i, y in enumerate(srv_ys, 1):
        primary.append({"id": f"DC-SRV-{i:02d}", "type": "server", "zone": "DC Compute",
                         "pos": (0.78 + (i % 2) * 0.10, y), "ip_zone": "datacenter", "is_shared": False,
                         "canonical_id": f"DC-SRV-{i:02d}"})
    ghost = []
    edges = [
    ...
    for i in range(1, n_srv + 1):
        edges.append({"src": "DC-AGG-SW-01", "dst": f"DC-SRV-{i:02d}",
                       "rel": "connected_to", "label": "VLAN", "is_cross": False})
    zones = [
        {"name": "DC Edge",        "color": "#FDEDEC", "bounds": (0.03, 0.20, 0.30, 0.78)},
        {"name": "DC Core",        "color": "#FEF9E7", "bounds": (0.32, 0.20, 0.50, 0.78)},
        {"name": "DC Aggregation", "color": "#EAFAF1", "bounds": (0.52, 0.28, 0.66, 0.70)},
        {"name": "DC Compute",     "color": "#EBF5FB", "bounds": (0.68, 0.14, 0.95, 0.88)},
    ]
    return primary, ghost, edges, zones
...
def _app_db_template(rng, scenario_idx, diagram_types):
    ...
    primary = [
        {"id": "APP-LB-01",   "type": "load_balancer", "zone": "App Ingress", "pos": (0.32, 0.48),
         "ip_zone": "appdb", "is_shared": False, "canonical_id": "APP-LB-01"},
        {"id": "SERVICE-API-01", "type": "service", "zone": "App Tier", "pos": (0.55, 0.70),
         "ip_zone": "appdb", "is_shared": False, "canonical_id": "SERVICE-API-01"},
        {"id": "DB-MASTER",   "type": "database",      "zone": "DB Tier",     "pos": (0.78, 0.42),
         "ip_zone": "appdb", "is_shared": False, "canonical_id": "DB-MASTER"},
        {"id": "DB-CACHE-01", "type": "server",        "zone": "DB Tier",     "pos": (0.78, 0.66),
         "ip_zone": "appdb", "is_shared": False, "canonical_id": "DB-CACHE-01"},
    ]
    app_ys = [0.30, 0.50, 0.62][:n_app]
    for i, y in enumerate(app_ys, 1):
        primary.insert(i, {"id": f"APP-{i:02d}", "type": "server", "zone": "App Tier",
                            "pos": (0.55, y), "ip_zone": "appdb", "is_shared": False,
                            "canonical_id": f"APP-{i:02d}"})
    for i in range(1, n_rep + 1):
        y = 0.20 + i * 0.58
        primary.append({"id": f"DB-REPLICA-{i:02d}", "type": "database", "zone": "DB Tier",
                         "pos": (0.91, y), "ip_zone": "appdb", "is_shared": False,
                         "canonical_id": f"DB-REPLICA-{i:02d}"})
    ghost = []
    edges = []
    if "datacenter_topology" in diagram_types:
        ghost.append({"id": "DC-CORE-SW-01", "type": "switch", "zone": "Network",
                       "pos": (0.10, 0.34), "is_shared": True, "canonical_id": "DC-CORE-SW-01"})
        ghost.append({"id": "DC-AGG-SW-01", "type": "switch", "zone": "Network",
                       "pos": (0.10, 0.62), "is_shared": True, "canonical_id": "DC-AGG-SW-01"})
        edges.append({"src": "DC-CORE-SW-01", "dst": "APP-LB-01", "rel": "routes_to",
                       "label": "HTTPS", "is_cross": True})
        edges.append({"src": "DC-AGG-SW-01", "dst": "APP-LB-01", "rel": "routes_to",
                       "label": "10G", "is_cross": True})
    if "shared_services_topology" in diagram_types:
        ghost.append({"id": "IAM-01", "type": "service", "zone": "Shared Services",
                       "pos": (0.94, 0.28), "is_shared": True, "canonical_id": "IAM-01"})
        ghost.append({"id": "DNS-01", "type": "service", "zone": "Shared Services",
                       "pos": (0.94, 0.62), "is_shared": True, "canonical_id": "DNS-01"})
    for i in range(1, n_app + 1):
        ...
        edges.append({"src": f"APP-{i:02d}", "dst": "DB-CACHE-01", "rel": "depends_on", "label": "API", "is_cross": False})
        edges.append({"src": f"APP-{i:02d}", "dst": "SERVICE-API-01", "rel": "depends_on", "label": "API", "is_cross": False})
    edges.append({"src": "SERVICE-API-01", "dst": "IAM-01", "rel": "depends_on", "label": "LDAP/IAM", "is_cross": True})
    edges.append({"src": "APP-01", "dst": "DNS-01", "rel": "depends_on", "label": "DNS", "is_cross": True})
    for i in range(1, n_rep + 1):
        edges.append({"src": "DB-MASTER", "dst": f"DB-REPLICA-{i:02d}",
                       "rel": "connected_to", "label": "DB Link", "is_cross": False})
    zones = [
        {"name": "Network",     "color": "#EBF5FB", "bounds": (0.04, 0.24, 0.22, 0.72)},
        {"name": "App Ingress", "color": "#FEF9E7", "bounds": (0.24, 0.24, 0.42, 0.72)},
        {"name": "App Tier",    "color": "#EAFAF1", "bounds": (0.44, 0.16, 0.66, 0.82)},
        {"name": "DB Tier",     "color": "#F5EEF8", "bounds": (0.68, 0.14, 0.96, 0.84)},
        {"name": "Shared Services Ref", "color": "#EBF5FB", "bounds": (0.88, 0.20, 0.98, 0.72)},
    ]
    return primary, ghost, edges, zones
...
def _shared_services_template(rng, scenario_idx, diagram_types):
    primary = [
        {"id": "AD-01",     "type": "service", "zone": "Identity",   "pos": (0.18, 0.42),
         "ip_zone": "shared", "is_shared": False, "canonical_id": "AD-01"},
        {"id": "IAM-01",    "type": "service", "zone": "Identity",   "pos": (0.36, 0.42),
        ...
        {"id": "MON-01",    "type": "service", "zone": "Monitoring", "pos": (0.76, 0.38),
         "ip_zone": "shared", "is_shared": False, "canonical_id": "MON-01"},
        {"id": "SYSLOG-01", "type": "server",  "zone": "Logging",    "pos": (0.76, 0.66),
         "ip_zone": "shared", "is_shared": False, "canonical_id": "SYSLOG-01"},
    ]
    ghost = []
    if "app_db_topology" in diagram_types:
        ghost.append({"id": "APP-01", "type": "server", "zone": "Consumers",
                      "pos": (0.92, 0.28), "is_shared": True, "canonical_id": "APP-01"})
    if "branch_topology" in diagram_types:
        ghost.append({"id": "BR-APP-01", "type": "server", "zone": "Consumers",
    ...
    edges = [
        ...
        {"src": "APP-01", "dst": "IAM-01",    "rel": "depends_on",   "label": "OAuth", "is_cross": True},
        {"src": "BR-APP-01", "dst": "DNS-01", "rel": "depends_on",   "label": "DNS", "is_cross": True},
    ]
    zones = [
        {"name": "Identity",   "color": "#F5EEF8", "bounds": (0.06, 0.22, 0.44, 0.62)},
        {"name": "DNS/NTP",    "color": "#EAFAF1", "bounds": (0.46, 0.18, 0.66, 0.76)},
        {"name": "Monitoring", "color": "#FEF9E7", "bounds": (0.68, 0.22, 0.84, 0.76)},
        {"name": "Consumers",  "color": "#EBF5FB", "bounds": (0.86, 0.16, 0.97, 0.78)},
    ]
    return primary, ghost, edges, zones
...
# DRAWING FUNCTIONS
# ============================================================
def _draw_device(ax, device_type, cx, cy, is_ghost=False, is_shared=False):
    R = DEVICE_RADIUS
    fc = DEVICE_FILL.get(device_type, "#888888")
    ec = DEVICE_EDGE_C.get(device_type, "#555555")
    alpha = GHOST_ALPHA if is_ghost else 0.88
    ls = "--" if (is_ghost or is_shared) else "-"
    lw = 1.8

    if device_type == "router":
        c = plt.Circle((cx, cy), R, fc=fc, ec=ec, lw=lw, ls=ls, alpha=alpha, zorder=3)
        ax.add_patch(c)
        for dx2, dy2 in [(0, -(R-5)), (0, R-5), (-(R-5), 0), (R-5, 0)]:
            ax.plot([cx, cx+dx2], [cy, cy+dy2], color=ec, lw=1.4, alpha=alpha, zorder=4)

    elif device_type == "switch":
        rect = mpatches.FancyBboxPatch((cx-R, cy-int(R*0.58)), R*2, int(R*1.16),
                                        boxstyle="round,pad=2", fc=fc, ec=ec,
                                        lw=lw, ls=ls, alpha=alpha, zorder=3)
        ax.add_patch(rect)
        h16 = int(R*1.16)
        for k in range(1, 4):
            yl = cy - int(R*0.58) + k * h16 // 4
            ax.plot([cx-R+5, cx+R-5], [yl, yl], color=ec, lw=1.0, alpha=alpha*0.65, zorder=4)

    elif device_type == "firewall":
        pts = [(cx, cy-R), (cx+R, cy-int(R*0.28)), (cx+int(R*0.68), cy+R),
               (cx-int(R*0.68), cy+R), (cx-R, cy-int(R*0.28))]
        s = plt.Polygon(pts, fc=fc, ec=ec, lw=lw, ls=ls, alpha=alpha, zorder=3)
        ax.add_patch(s)
        o = int(R*0.35)
        ax.plot([cx-o, cx+o], [cy-o+4, cy+o-4], color="#ffffff", lw=1.5, alpha=alpha*0.7, zorder=4)
        ax.plot([cx+o, cx-o], [cy-o+4, cy+o-4], color="#ffffff", lw=1.5, alpha=alpha*0.7, zorder=4)

    elif device_type == "server":
        rect = mpatches.FancyBboxPatch((cx-R, cy-int(R*0.72)), R*2, int(R*1.44),
                                        boxstyle="square,pad=0", fc=fc, ec=ec,
                                        lw=lw, ls=ls, alpha=alpha, zorder=3)
        ax.add_patch(rect)
        for k in range(3):
            yl = cy - int(R*0.72) + 7 + k*10
            ax.plot([cx-R+4, cx+R-4], [yl, yl], color=ec, lw=0.9, alpha=alpha*0.55, zorder=4)
        led = plt.Circle((cx+R-7, cy-int(R*0.72)+5), 3, fc="#00ff80", ec=None, alpha=alpha, zorder=5)
        ax.add_patch(led)
        ...
        ax.add_patch(mpatches.Ellipse((cx, cy-int(R*0.55)), ew*2, int(R*0.45),
                                       fc=ec, ec=ec, lw=lw, ls=ls, alpha=alpha, zorder=4))
        ax.add_patch(mpatches.Ellipse((cx, cy+int(R*0.80)), ew*2, int(R*0.45),
                                       fc=fc, ec=ec, lw=lw, ls=ls, alpha=alpha*0.7, zorder=4))

    elif device_type == "load_balancer":
        pts = [(cx, cy-R), (cx+R, cy), (cx, cy+R), (cx-R, cy)]
        d = plt.Polygon(pts, fc=fc, ec=ec, lw=lw, ls=ls, alpha=alpha, zorder=3)
        ax.add_patch(d)
        ...
        for dx2, dy2, rf in [(-int(R*0.38), int(R*0.10), R*0.55),
                               ...
                               (0,           -int(R*0.15), R*0.60)]:
            ax.add_patch(plt.Circle((cx+dx2, cy+dy2), rf, fc=fc, ec=None, alpha=alpha*0.75, zorder=3))
        ax.add_patch(plt.Circle((cx, cy), R*0.94, fc="none", ec=ec, lw=lw, ls=ls, alpha=alpha, zorder=4))

    elif device_type == "service":
        hex_pts = [(cx + R*math.cos(math.radians(60*i - 30)),
                    cy + R*math.sin(math.radians(60*i - 30))) for i in range(6)]
        ax.add_patch(plt.Polygon(hex_pts, fc=fc, ec=ec, lw=lw, ls=ls, alpha=alpha, zorder=3))
        ax.add_patch(plt.Circle((cx, cy), int(R*0.30), fc="#ffffff", ec=None,
                                  alpha=alpha*0.65, zorder=4))

    else:
        ax.add_patch(plt.Circle((cx, cy), R, fc=fc, ec=ec, lw=lw, ls=ls, alpha=alpha, zorder=3))


def _draw_connector(ax, x1, y1, x2, y2, label="", is_cross=False):
    color = CROSS_COLOR if is_cross else CONNECTOR_COLOR
    ls = (0, (4, 3)) if is_cross else "-"
    dx = x2 - x1; dy = y2 - y1
...
def _draw_zone(ax, name, x0, y0, x1, y1, color):
    ...
    ax.text(x0+10, y0+16, name.upper(), fontsize=7.2, color=ZONE_LABEL_COLOR,
            fontweight="bold", alpha=0.95, zorder=5, fontfamily="monospace")


def _draw_title(ax, diagram_type, scenario_id):
    ax.add_patch(mpatches.Rectangle((0, 0), WIDTH, TITLE_H,
                                    fc=HEADER_BLUE, ec="none", zorder=6))
    ax.plot([0, WIDTH], [TITLE_H, TITLE_H], color="#0D47A1", lw=1.5, zorder=7)
    ...
    ax.text(22, 52, f"Diagram Intelligence V3 | Scenario {scenario_id} | scenario-native local view",
            ha="left", va="center", fontsize=8.5, color=HEADER_SUB, zorder=8)
    ax.text(WIDTH-18, 52, diagram_type, ha="right", va="center",
            fontsize=8, color=HEADER_SUB, alpha=0.95, zorder=8)
    badge = DIAGRAM_TYPE_COLORS.get(diagram_type, "#888888")
    ax.add_patch(plt.Circle((WIDTH-18, 24), 5, fc=badge, ec="#FFFFFF", lw=0.8, zorder=8))


def _draw_metadata_box(ax, scenario_id, diagram_type, primary_nodes, ghost_nodes, edges):
    x0 = WIDTH - 205
    y0 = TITLE_H + 12
    ax.add_patch(mpatches.FancyBboxPatch(
        (x0, y0), 190, 180, boxstyle="round,pad=4",
        fc="#F8F9FA", ec="#E0E0E0", lw=1.0, zorder=4))
    ax.text(x0+10, y0+18, "SITE INFORMATION", fontsize=8.5,
            color=HEADER_BLUE, fontweight="bold", zorder=5)
    rows = [
        ("Scenario", scenario_id),
        ("Diagram", diagram_type.replace("_", " ")),
        ("Nodes", str(len(primary_nodes) + len(ghost_nodes))),
        ("Edges", str(len(edges))),
        ("Shared", str(sum(1 for n in primary_nodes + ghost_nodes if n.get("is_shared")))),
        ("Version", "v3"),
    ]
    for i, (k, v) in enumerate(rows):
        y = y0 + 42 + i * 20
        ax.text(x0+10, y, k, fontsize=7.5, color="#777777", zorder=5)
        ax.text(x0+72, y, v[:18], fontsize=7.5, color=TEXT_COLOR, zorder=5)


def _draw_legend(ax):
    x0, y0 = 18, HEIGHT - 182
    ax.add_patch(mpatches.FancyBboxPatch(
        (x0, y0), 155, 145, boxstyle="round,pad=4",
        fc="#FFFFFF", ec="#E0E0E0", lw=1.0, alpha=0.96, zorder=4))
    ax.text(x0+10, y0+18, "LEGEND", fontsize=8.5, color=HEADER_BLUE,
            fontweight="bold", zorder=5)
    entries = ["router", "switch", "firewall", "server", "database", "load_balancer", "cloud_or_wan", "service"]
    for i, cls in enumerate(entries):
        y = y0 + 37 + i * 13
        ax.add_patch(plt.Circle((x0+16, y-3), 4, fc=DEVICE_FILL[cls], ec=DEVICE_EDGE_C[cls],
                                lw=0.6, zorder=5))
        ax.text(x0+27, y, cls, fontsize=6.5, color=LABEL_COLOR, va="center", zorder=5)


def draw_diagram(scenario_id, diagram_type, primary_nodes, ghost_nodes, edges, zones, out_path):
    """Render diagram to PNG. Returns {node_id: (cx, cy)} pixel map."""
    ...
    ax.add_patch(mpatches.Rectangle((12, TITLE_H+8), WIDTH-230, HEIGHT-TITLE_H-54,
                                    fc=PAGE_BG, ec="#ECEFF1", lw=0.8, zorder=0))
    _draw_title(ax, diagram_type, scenario_id)
    _draw_metadata_box(ax, scenario_id, diagram_type, primary_nodes, ghost_nodes, edges)
    _draw_legend(ax)
    ...
    for z in zones:
        ...
        px0 = int(x0n*WIDTH); py0 = int(DRAW_Y0 + y0n*DRAW_H)
        px1 = int(x1n*WIDTH); py1 = int(DRAW_Y0 + y1n*DRAW_H)
        _draw_zone(ax, z["name"], px0, py0, px1, py1, z["color"])

    interface_names = ["Gi0/1", "eth0", "ge-0/0/1", "Gi0/2", "eth1"]
    for edge_idx, e in enumerate(edges):
        if e["src"] in node_pixels and e["dst"] in node_pixels:
            x1p, y1p = node_pixels[e["src"]]
            x2p, y2p = node_pixels[e["dst"]]
            _draw_connector(ax, x1p, y1p, x2p, y2p,
                            label=e.get("label", ""), is_cross=e.get("is_cross", False))
            iface_a = interface_names[edge_idx % len(interface_names)]
            iface_b = interface_names[(edge_idx + 2) % len(interface_names)]
            ax.text(x1p + 8, y1p + 10, iface_a, ha="left", va="center",
                    fontsize=5, color=ZONE_LABEL_COLOR, alpha=0.8,
                    fontfamily="monospace", zorder=5)
            ax.text(x2p - 8, y2p - 10, iface_b, ha="right", va="center",
                    fontsize=5, color=ZONE_LABEL_COLOR, alpha=0.8,
                    fontfamily="monospace", zorder=5)

    for n in ghost_nodes:
        cx, cy = node_pixels[n["id"]]
        _draw_device(ax, n["type"], cx, cy, is_ghost=True, is_shared=True)
        ax.text(cx, cy+DEVICE_RADIUS+13, n["id"], ha="center", va="top",
                fontsize=7, color=TEXT_COLOR, alpha=0.78, zorder=6, fontfamily="monospace",
                bbox=dict(fc="#FFFFFF", ec="none", pad=0.6, alpha=0.75))
        ax.text(cx, cy-DEVICE_RADIUS-7, "[shared]", ha="center", va="bottom",
                fontsize=5.5, color=CROSS_COLOR, alpha=0.65, zorder=6)

    for n in primary_nodes:
        cx, cy = node_pixels[n["id"]]
        _draw_device(ax, n["type"], cx, cy, is_ghost=False, is_shared=n.get("is_shared", False))
        ax.text(cx, cy+DEVICE_RADIUS+13, n["id"], ha="center", va="top",
                fontsize=8, color=TEXT_COLOR, fontweight="bold", zorder=6, fontfamily="monospace",
                bbox=dict(fc="#FFFFFF", ec="none", pad=0.7, alpha=0.82))
        ip = n.get("ip_address", "")
        if ip:
            ax.text(cx, cy-DEVICE_RADIUS-6, ip, ha="center", va="bottom",
                    fontsize=6, color=ZONE_LABEL_COLOR, alpha=0.95, zorder=6, fontfamily="monospace",
                    bbox=dict(fc="#FFFFFF", ec="none", pad=0.5, alpha=0.70))
        if n.get("is_shared"):
            ax.text(cx+DEVICE_RADIUS+4, cy-DEVICE_RADIUS, "*", ha="left", va="top",
                    fontsize=9, color=CROSS_COLOR, fontweight="bold", zorder=6)

    ax.plot([0, WIDTH], [HEIGHT-34, HEIGHT-34], color="#E0E0E0", lw=1.0, zorder=6)
    ax.text(16, HEIGHT-14, f"InfraGraph AI | Diagram Intelligence V3 | {scenario_id} | {diagram_type}",
...
def make_annotation(scenario_id, diagram_id, diagram_type, image_path,
    ...
    R = DEVICE_RADIUS + BBOX_PAD
    objects = []
    for n in primary_nodes:
        cx, cy = node_pixels[n["id"]]
        x1 = max(0, cx-R); y1 = max(0, cy-R)
        x2 = min(WIDTH, cx+R); y2 = min(HEIGHT, cy+R)
        objects.append({
            "object_id":       n["id"],
            "class_name":      n["type"],
            "bbox":            [x1, y1, x2, y2],
    ...
    for n in primary_nodes + ghost_nodes:
        ...
        if ip:
            ...
            text_blocks.append({
                "text": ip,
                "bbox": [cx-iw, cy-DEVICE_RADIUS-16, cx+iw, cy-DEVICE_RADIUS-4],
    ...
    for z in zones:
        ...
        text_blocks.append({
            "text": z["name"],
            "bbox": [zx, zy, min(WIDTH, zx + zw), min(HEIGHT, zy + 16)],
    ...
    for i, e in enumerate(edges):
        if e["src"] in all_pos and e["dst"] in all_pos:
            ...
            src_iface = interface_names[i % len(interface_names)]
            dst_iface = interface_names[(i + 2) % len(interface_names)]
            connectors.append({
                ...
                "style":         "dashed" if e.get("is_cross") else "solid",
            })
            if e.get("label"):
            ...
            for txt, tx, ty, align in [
                (src_iface, sx + 8, sy + 10, "left"),
                (dst_iface, dx2 - 8, dy2 - 10, "right"),
            ]:
                iw = len(txt) * 4
                if align == "left":
                    bbox = [tx, ty - 6, tx + iw, ty + 6]
                else:
                    bbox = [tx - iw, ty - 6, tx, ty + 6]
                text_blocks.append({
                    "text": txt,
                    "bbox": bbox,
                    "text_type": "interface_label",
                })
...
def make_preview_contact_sheet(scenario_id, diagram_types, diagram_dir, out_path):
    ...
    fig, axes = plt.subplots(rows, cols, figsize=(cols*7.4, rows*5.0),
                              facecolor="#F5F5F5")
    ...
    for idx, dtype in enumerate(diagram_types):
        ...
        img_path = diagram_dir / f"{dtype}.png"
        arr = _load_png_as_array(img_path)
        if arr is not None:
        ...
        if lg_path.exists():
            try:
                ...
                ax.text(0.99, 0.04, count_text, transform=ax.transAxes,
                        ...
                        bbox=dict(boxstyle="round,pad=0.2", fc="#FFFFFFDD",
                                  ec="#B0BEC5", lw=0.5),
                        zorder=6)
            except Exception:
                pass
...
def make_preview_stitching_story(scenario_id, diagram_types, diagram_dir, out_path):
    ...
    for sx in [6.95, 13.55]:
        ax.annotate("", xy=(sx+0.42, 6.2), xytext=(sx, 6.2),
                    arrowprops=dict(arrowstyle="-|>", color="#37474F", lw=3.0,
                                    mutation_scale=22))
...
def make_preview_stitching_story_hero(scenario_id, diagram_types, diagram_dir, out_path):
    ...
    _draw_rca_summary_box(ax, alerts_data, 15.00, 0.82, 7.85, 1.95,
                          title="RCA Summary")
    ax.text(19.0, 2.26,
...
def _draw_enterprise_cluster_graph(ax, enterprise_graph, alerts_data, x0, y0, w, h):
    ...
    for e in cross_edges:
        ...
        s, t = e.get("source"), e.get("target")
        p1 = all_panel_pos.get((sd, s)) or next((xy for (did, nid), xy in all_panel_pos.items() if nid == s), None)
        p2 = all_panel_pos.get((td, t)) or next((xy for (did, nid), xy in all_panel_pos.items() if nid == t), None)
        if p1 and p2:
            ...
            if lbl:
                ax.text(mx, my, lbl[:12], fontsize=5.8, color="#01579B",
                        ha="center", va="center",
                        bbox=dict(fc="#E1F5FE", ec="#B3E5FC", pad=0.8),
                        zorder=9)
    ...
    if key_path:
        ...
        for a, b in zip(path, path[1:]):
            x1, y1 = node_pos[a]
            x2, y2 = node_pos[b]
            ax.plot([x1, x2], [y1, y2], color="#D32F2F", lw=3.2,
                    alpha=0.88, zorder=8)
        if path:
    ...
    for i, (lbl, clr) in enumerate(legend):
        ...
        ax.text(legend_x + 0.28, yy, lbl, fontsize=6.0, color=LABEL_COLOR,
                va="center", zorder=11)
...
def make_preview_rca_overlay(scenario_id, enterprise_graph, alerts_data, out_path):
    ...
    _draw_rca_summary_box(ax, alerts_data, 12.05, 5.40, 3.45, 3.45,
                          title="Root Cause")
    ax.add_patch(mpatches.FancyBboxPatch(
    ...
    for alert in (alerts_data or {}).get("alerts", [])[:7]:
        color = {"critical": "#E53935", "high": "#FB8C00", "warning": "#FDD835"}.get(alert.get("severity"), "#90A4AE")
...
def generate_scenario(scenario_idx, split, out_root, master_rng):
    ...
    # Keep the first scenario as the deterministic demo hero with the complete
    # five-diagram enterprise chain.
    if scenario_idx == 0:
...
def make_global_preview_sheet(out_root, all_metadata, preview_name, out_name, max_items=12):
    ...
    for meta in all_metadata:
        preview_path = (
            out_root / "scenarios" / meta["split"] / meta["scenario_id"] / preview_name
        )
    ...
    for idx, (meta, preview_path) in enumerate(candidates):
        ...
        if arr is not None:
            ax.imshow(arr, aspect="auto")
        ax.set_title(
            f"{meta['scenario_id']} | {meta['num_diagrams']} diagrams",
            fontsize=8,
            color=TEXT_COLOR,
            pad=4,
        )
...
def main():
    ...
    p.add_argument("--out",           type=str,   default="./datasets/diagram_v3_enterprise")
    ...
    make_global_preview_sheet(
        out_root,
        all_metadata,
        "preview_stitching_story.png",
        "stitching_story_contact_sheet.png",
    )
    make_global_preview_sheet(
        out_root,
        all_metadata,
        "preview_enterprise_graph.png",
        "enterprise_contact_sheet.png",
    )
